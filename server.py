...
#     parser.add_argument("--model", type=str, default="customcnn", choices=["mobilenetv3", "hybridmodel", "resnet50", "customcnn", "hybridswin", "densenet121", "shufflenetv2", "squeezenet11", "regnety400"], help="Model architecture")
...
def get_init_parameters(model_name: str, num_classes: int) -> fl.common.Parameters:
    """Get initial model parameters for FL server"""
    try:
        # Assumes 'models' is a directory in the same folder as this script
        models_dir = os.path.join(os.path.dirname(__file__), "models")
        if models_dir not in sys.path:
            sys.path.insert(0, models_dir)
        
        # Now it can find get_model
        model = get_model(model_name, num_classes, pretrained=False)
        parameters = [val.cpu().numpy() for _, val in model.state_dict().items()]
...
class MedicalFLStrategy(fl.server.strategy.FedAvg):
    ...
    def _calculate_fit_metrics(self, results):
        ...
        current_round = len(self.history["round"]) + 1 # Note: This might be off by 1 if called after history append
        self.client_metrics_history[current_round] = client_metrics_list
...
def create_server_strategy(
    *,
    ...
    return MedicalFLStrategy(
        ...
        fraction_evaluate=fraction_evaluate,
        min_fit_clients=min_clients,
        min_evaluate_clients=1, # Allow evaluation even if only 1 client is available
        min_available_clients=min_clients,
        on_fit_config_fn=lambda server_round: fit_config(server_round, local_epochs),
        on_evaluate_config_fn=evaluate_config,
        initial_parameters=initial_parameters,
...
def main():
    ...
    # This 'strategy' variable will be captured in the 'finally' block
    strategy = None 
    try:
        strategy = create_server_strategy(
            min_clients=args.min_clients,
        ...
        # This block will run even if an error occurs (or on KeyboardInterrupt)
        if strategy: # Only save if strategy was successfully initialized
            try:
                strategy.save_final_results()
                strategy.save_last_model()
                if strategy.history["round"]:
            ...
            logger.warning("Server strategy was not initialized. No results to save.")

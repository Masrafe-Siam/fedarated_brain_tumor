...
# import cv2
# import time
# import grpc  # for robust reconnect handling
...
#         if isinstance(m, nn.Conv2d):
#             last = m
#     return last  # return None if not found
...
#         # pick last conv once (for Grad-CAM, etc.); optional
#         self.target_layer = _find_last_conv(self.model)
...
#     def get_parameters(self, config: Dict = None) -> List[np.ndarray]:
#         """Return model parameters as numpy arrays (order must match state_dict keys)."""
#         return [p.detach().cpu().numpy() for p in self.model.state_dict().values()]
    
#     def set_parameters(self, parameters: List[np.ndarray]) -> None:
#         """Tolerant parameter loading that preserves dtype and ignores missing buffers."""
#         own_state = self.model.state_dict()
#         keys = list(own_state.keys())
#         incoming = OrderedDict()
#         for k, v in zip(keys, parameters):
#             t = torch.tensor(v, dtype=own_state[k].dtype)  # keep dtype
#             incoming[k] = t
#         # Merge, but don't fail on missing keys/buffers
...
#         # XAI: optional if we have a conv layer
...
#         # Final metrics (only scalars to keep gRPC payload small)
#         metrics = {
...
#         # Evaluate on test set
#         test_metrics = self.trainer.evaluate(self.test_loader)
#         # Optional: quick XAI probe during evaluate as well (no file saving)
...
#         return (float(test_metrics.get("loss", 0.0)), 
...
# #     return next((m for m in reversed(list(module.modules())) if isinstance(m, nn.Conv2d)), None)

# # def overlay_on_gray(img_u8: np.ndarray, heat: np.ndarray, alpha: float = 0.35) -> np.ndarray:
# #     H, W = img_u8.shape
# #     heatmap = cv2.applyColorMap((cv2.resize(heat, (W, H)) * 255).astype(np.uint8), cv2.COLORMAP_JET)
# #     base = cv2.cvtColor(img_u8, cv2.COLOR_GRAY2BGR)
# #     return cv2.addWeighted(base, 1.0, heatmap, alpha, 0)

# # def deletion_curve_scores(model: nn.Module, x: torch.Tensor, heat: np.ndarray, steps: int = 10) -> List[float]:
# #     device = next(model.parameters()).device
# #     x = x.clone().to(device)
# #     with torch.no_grad():
# #         cls = int(model(x)[0].argmax().item())
# #         scores = [model(x)[0, cls].item()]
...
# #         self.model.zero_grad(set_to_none=True)
# #         logits = self.model(x)
# #         class_idx = class_idx if class_idx is not None else int(torch.argmax(logits, dim=1).item())
# #         logits[0, class_idx].backward(retain_graph=True)
# #         A, dA = self.A[0], self.dA[0]
# #         w = dA.mean(dim=(1,2))
# #         cam = torch.relu((w[:, None, None] * A).sum(dim=0)).detach().cpu().numpy()
...
# # # ===================== FL CLIENT =====================
# # class MedicalFLClient(fl.client.NumPyClient):
# #     def __init__(self, client_id: int, data_dir: str, device: torch.device, model_name: str = "customcnn",
# #                  num_classes: int = 3, batch_size: int = 32, local_epochs: int = 8):
# #         self.client_id = client_id
# #         self.data_dir = data_dir
# #         self.device = device
# #         self.num_classes = num_classes
...
# #         # Setup directories
# #         self.client_root = os.path.join(RESULTS_BASE_DIR, f"client_{client_id}")
# #         self.ckpt_dir, self.log_dir, self.xai_dir, self.pred_dir, self.metrics_dir = [
# #             os.path.join(self.client_root, d) for d in ["checkpoints", "logs", "xai", "predictions", "metrics"]
# #         ]
# #         for d in [self.ckpt_dir, self.log_dir, self.xai_dir, self.pred_dir, self.metrics_dir]:
# #             os.makedirs(d, exist_ok=True)

# #         # Load model
# #         self.model = get_model(model_name, num_classes, pretrained=True).to(device)
# #         self.target_layer = find_last_conv(self.model)
# #         if self.target_layer is None:
# #             logging.warning("No Conv2d layer found for Grad-CAM. XAI probe will be skipped.")

# #         # Data loaders
# #         logging.info(f"Client {client_id}: Loading data from {data_dir}")
# #         self.train_loader, self.val_loader, self.test_loader = create_data_loaders(
# #             data_dir, batch_size, 0.8, 0.1, 0.1, image_size=(224,224), num_workers=0
# #         )

# #         # Class weights
# #         self.class_weights = get_class_weights(self.train_loader)
# #         logging.info(f"Client {client_id}: Class weights: {self.class_weights.tolist() if hasattr(self.class_weights, 'tolist') else self.class_weights}")
...
# #         own_state = self.model.state_dict()
# #         for k, v in zip(own_state.keys(), parameters):
# #             own_state[k].copy_(torch.tensor(v, dtype=own_state[k].dtype))
...
# #         # train, compute metrics, XAI probe, save model
...
# #     return MedicalFLClient(client_id, data_dir, device, model_name=model_name)

# # # ===================== RUN FLOWER =====================
# # def run_flower(server_address: str, client: MedicalFLClient):
# #     while True:
# #         try:
...
# #         except grpc.RpcError as e:
# #             if e.code() == grpc.StatusCode.UNAVAILABLE:
# #                 logging.warning("Server UNAVAILABLE. Reconnecting in 5s...")
# #                 time.sleep(5)
# #             else:
# #                 raise
...
# #     if args.train_local:
# #         logging.info("Running local training (no FL server)")
# #         updated_params, _, train_metrics = client.fit(client.get_parameters(), config={})
# #         _, _, test_metrics = client.evaluate(updated_params, config={})
# #         logging.info(f"Train metrics: {train_metrics}")
# #         logging.info(f"Test metrics: {test_metrics}")
# #         return

# #     logging.info(f"Starting FL client {args.client_id} connecting to {args.server_address}")
# #     run_flower(args.server_address, client)
